---
title: Anatolian Hieroglyphs (Luwian Hieroglyphs, Hittite Hieroglyphs)
description: Top-level script page
sidebar:
    hidden: true
lastUpdated: 2026-08-26
scrpropcode: Hluw
scrproptype: logo-syllabary
scrpropname: Anatolian Hieroglyphs (Luwian Hieroglyphs, Hittite Hieroglyphs)
scrpropshortname: Anatolian Hieroglyphs
scrpropstatus: Historical
scrpropregion: Middle Eastern
scrpropdirection: boustrophedon
scrpropbaseline: unspecified
scrpropcase: no
scrpropwspace: none
scrpropisonumkey: 080 (hieroglyphic/cuneiform)
scrpropotcode: hluw
scrpropsortname: Anatolian Hierglyphs
---

import ScriptDetails from '../../../../components/ScriptDetails.astro';
import WsList from '../../../../components/WsList.astro';
import ArticlesList from '../../../../components/ArticlesList.astro';
import SourceLinksList from '../../../../components/SourceLinksList.astro';
import BibList from '../../../../components/BibList.astro';
import Attribution from '/src/components/Attribution.astro';
import CaptionText from '/src/components/CaptionText.astro';

![Anatolian Hieroglyphs sample](../images/hluw-sample.png)
<Attribution
    type="Image"
    copyholder="voyageAnatolia.blogspot.com"
    copyurl="https://voyageanatolia.blogspot.com/"
    copyyears="2010"
    license="CC BY-NC-SA 2.0"
    licenseurl="https://creativecommons.org/licenses/by-nc-sa/2.0/deed.en"
    author=""
    source="Flickr"
    sourceurl="https://www.flickr.com/photos/fotogezi/4501752277/"
/>

## Script details

<ScriptDetails />

## Script description

Anatolian Hieroglyphs were used for writing the Luwian language from the second millennia BC until the early 7th century BC.

<details>
<summary>Read the full description...</summary>The script has also been called ‘Hittite Hieroglyphs’ as it was once thought that they had been used for writing the Hittite language; however we now know that was not the case.

There are approximately five hundred signs attested in inscriptions on stone and lead. These have both syllabic and logographic components. Words could be written entirely with logographs (in particular at the earlier period), or with phonetic complements in a variety of configurations.
</details>

## Languages that use this script

<WsList script='Hluw' wsMax='5' />

## Unicode status

In The Unicode Standard, Anatolian Hieroglyphs script implementation is discussed in [Chapter 11 Cuneiform and Hieroglyphs](https://www.unicode.org/versions/latest/core-spec/chapter-11/#G27920).

- [Full Unicode status for Anatolian Hieroglyphs](/scrlang/unicode/hluw-unicode)



## Resources

<ArticlesList tag='script-hluw' header='Related articles' />

<SourceLinksList tag='script-hluw' header='External links' entrytype='online' />

<BibList tag='script-hluw' header='Bibliography' entrytype='non-online' />
