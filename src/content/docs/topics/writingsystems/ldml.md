...
The "types" attribute consists of a series of roles describing the context in which the font would be used, and the level of priority that font should be given in that context when compared to other listed fonts. Priority is indicated by an integer less than or equal to 100, with the lowest values carrying the highest priority. A font with type="default=1" is the highest priority possible, while one with type="defualt=100" is the lowest priority possible. A font can have multiple roles with individual priority values, such as "default=3 ui=5". The numbers themselves carry no meaning except as a way to compare the fonts in a list; if there are two fonts, and one has a priority value of 3, it does not matter if a lower priority font has a priority of 4 or 40, so long as it is a higher number and less than or equal to 100. If a role is listed in the type attribute, but has no priority value, that value is assumed to be 1 (in other words, type="default" is equivalent to type="default=1"). If no type attribute is listed at all, the font is given an implied "default=100" priority, aka the lowest priority possible. 
...
The CLDR stores the rules for non-default parent locales in their supplemental data file, which is described on [page 6 of UTS#35][uni-utr35-parent-locales]. 
