...
```
Please note the `:char` directive is a little more limited, and cannot accept
certain directive meta characters `[`,`]`, `{`,`}`, or whitespace.
...
```mdx
:usv[0635] and :usv[0635]{usv char}
```
>Result: :usv[0635] and :usv[0635]{usv char}

```mdx
:char[@] and :char[@]{usv char}
```
>Result: :char[@] and :char[@]{usv char}
