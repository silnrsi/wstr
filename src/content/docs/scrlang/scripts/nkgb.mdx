---
title: Naxi Geba (na²¹ɕi³³ gʌ²¹ba²¹, 'Na-'Khi ²Ggŏ-¹baw, Nakhi Geba)
description: Top-level script page
sidebar:
    hidden: true
lastUpdated: 2026-08-26
scrpropcode: Nkgb
scrproptype: syllabary
scrpropname: Naxi Geba (na²¹ɕi³³ gʌ²¹ba²¹, 'Na-'Khi ²Ggŏ-¹baw, Nakhi Geba)
scrpropshortname: Naxi Geba script
scrpropstatus: Historical
scrpropregion: Central Asian
scrpropdirection: LTR
scrpropbaseline: bottom
scrpropcase: no
scrpropwspace: unspecified
scrpropisonumkey: 420 (syllabic)
scrpropotcode: unspecified
scrpropsortname: Naxi Geba (naxi gaba)
---

import ScriptDetails from '../../../../components/ScriptDetails.astro';
import WsList from '../../../../components/WsList.astro';
import ArticlesList from '../../../../components/ArticlesList.astro';
import SourceLinksList from '../../../../components/SourceLinksList.astro';
import BibList from '../../../../components/BibList.astro';
import Attribution from '/src/components/Attribution.astro';
import CaptionText from '/src/components/CaptionText.astro';

![Naxi Geba script sample](../images/nkgb-sample.jpg)
<CaptionText text="Naxi scripture in syllabic Geba script."/>
<Attribution
    type="Image"
    copyholder=""
    copyurl=""
    copyyears=""
    license="Public Domain"
    licenseurl=""
    author="Daderot"
    source="Wikimedia"
    sourceurl="https://commons.wikimedia.org/wiki/File:Manuscripts_in_the_Yunnan_Nationalities_Museum_-_DSC04026.JPG"
/>

## Script details

<ScriptDetails />

## Script description

Nakhi Geba (also called 'Na-'Khi ²Ggŏ-¹baw or Naxi Geba) is one of three scripts used for writing the Naxi language, a Tibeto-Burman language spoken by approximately 310,000 people in the Chinese provinces of Yunnan and Sichuan, and in Tibet.

<details>
<summary>Read the full description...</summary>Naxi has also been written in the Latin and, more famously, the Dongba scripts.

Nakhi Geba is thought to have been developed around 1200-1250 AD but was never widely used; its primary use was the transcription of religious mantras. There are few remaining texts in the script, and little is known about it. It was a syllabary, and the shapes of the letters appear to have been variously based on Chinese and Dongba characters, with some original designs.
</details>

## Languages that use this script

:::note
A status of _obsolete_ indicates that the writing system is no longer in use for that language; the language may still be spoken.
:::

<WsList script='Nkgb' wsMax='5' />

## Unicode status

The Naxi Geba script is not yet in Unicode. The script has a tentative allocation at U+1AD00..U+1AFCF in the [Roadmap to the SMP](http://www.unicode.org/roadmaps/smp/) for the Unicode Standard. There seems to be disagreement on whether Naxi Dongba and Nakhi Geba should be considered the same script (see also the [Unicode Status (Naxi Dongba)](/scrlang/unicode/nkdb-unicode/) entry).

- [Full Unicode status for Naxi Geba](/scrlang/unicode/nkgb-unicode)



## Resources

<ArticlesList tag='script-nkgb' header='Related articles' />

<SourceLinksList tag='script-nkgb' header='External links' entrytype='online' />

<BibList tag='script-nkgb' header='Bibliography' entrytype='non-online' />

