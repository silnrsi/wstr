...
![reordering-and-data-storage-order-1](images/reordering-and-data-storage-order-1)Notice that the order in which the characters are written in Devanagari does not reflect the order in which they are spoken. Specifically, the _i_ is written before the _ch_, even though it is pronounced after it.
