...
This is a visual overview of the workflow, see next paragraph for more details of each step.

```mermaid
graph TB
    A[/Spin up container/]  e1@==> B[/Navigate to font project/]
    B  e2@==> C[/Pull outstanding changes/]
    C  e3@==> D@{ shape: processes, label: "Modify font sources - CLI tools or GUI font editor after preglyphs" }
    D  e4@==> E[/Save and normalize with preflight/]
    E  e5@==> F[/Run build/]
    F  e6@==> G@{ shape: processes, label: "Run tests, QA and regression tools" }
    G  e7@==> H[/Review results/]
    H  e8@==> I@{ shape: processes, label: "Commit/Push" }
    H  e9@==> D
    I  e1@==> J[/Exit and spin down container/]
    e4@{ animate: true }
    e5@{ animate: true }
    e6@{ animate: true }
    e7@{ animate: true }
...
- Use _Smith_ commands to:
  - Clean the build from any remaining artefacts and log files (`smith distclean`)
  - Configure the build  (`smith configure`)
  - Build the fonts  (`smith build`)
  - Prepare test files and run test suites (`smith alltests`, `smith fbchecks`,  `smith fontspector`)
...
- Make sure copyright notices are correctly conveyed or added to
- Tag the last commit to reflect the release version
- Build the fonts using `smith zip`, `smith tarball`, or `smith release`
- Make a release on Github, uploading the release artefacts from `results/releases/`
