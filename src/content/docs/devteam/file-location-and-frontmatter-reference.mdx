...
### Filenames for other files (PDF, ZIP, etc.)

Files that are not pages or images and can either be viewed (.pdf) or downloaded (.zip and others) should have filenames that reflect the content and purpose of the file. This ensures that downloaded files end up with sensible, user-friendly names. Like other filenames they should be all lowercase with no spaces, using hyphens to separate words. There is no need to make the filename relate to the page that links to it.
...
**Downloadable files or viewable PDFs** need to be placed in a special folder outside the main `src/content/docs` folder. They should be placed in `public/files`. Subfolders are not needed unless the number of files being added is a large number and there is some benefit to having them all in a separate folder.
