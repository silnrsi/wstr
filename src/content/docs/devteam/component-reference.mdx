...
|license| The name of the license for the resource. If it is "Public Domain" or "CC0 1.0" then `copyholder`, `copyyears` and `copyurl` properties are ignored. |
...
|copyholder| Copyright holder name. Only required when `license` is not "Public Domain" or "CC0 1.0". |
|copyyears| Copyright years, can be a single year, or range of years. Only required when `license` is not "Public Domain" or "CC0 1.0". |
|copyurl| Optional URL that links to copyright holder information, when present it will format the `copyholder` as a link. Only required when `license` is not "Public Domain" or "CC0 1.0". |
