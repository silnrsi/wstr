...
Heading 1 is only used by the page title. Anchors are automatically generated for all headings. The heading title can be added to the page URL with a hash, replacing spaces with hyphens, removing punctuation, and making it all lowercase. A heading of 'Image references' would become `#image-references`.
...
To add a caption to an image, use the custom **CaptionText** component - see the [Component Reference](/devteam/component-reference#captiontext-component). For example:

![This is alt text](images/9800-small-image.png)
<CaptionText text="This is the caption text for the image above. It supports **basic** markdown styling, however, some styling (e.g. italic) may be overridden by the site-wide styling."/>
...
Note that use of this component requires that the file be in `.mdx` format rather than `.md`, and that the component is imported at the top of the file.

## Attributions

To add a formal attribution to an image or to a whole article, use the custom **Attribution** component - see the [Component Reference](/devteam/component-reference#attribution-component). For example:
...
**External links** should always use the full URL using a named references in [this example][otspec].
```md
...
name, as in [this direct link to the HImage references section](#image-references). The anchor name
format is a hash plus the heading name, lowercased, with spaces replaced by
hyphens, and other punctuation removed as in `#image-references`. There is no need to
...
[adbp]: /topics/computing/app-development-best-practice

## Downloadable or viewable files

Links to downloadable files (ZIP, etc.) or viewable files (PDF) should use raw HTML anchors rather than markdown-style links. The `href` should point to the file in the `public/files` folder using the format `/files/filename`. (See [File locations](/devteam/file-location-and-frontmatter-reference#file-locations) for details on where to place the files themselves.)

**Viewable PDFs** will by default be opened in the user's browser or PDF viewer. The anchor needs an additional `class="file-pdf"` added. This enables us to add a PDF icon after the link name to indicate to users that the linked file is a PDF. For example, <a href="/files/test-pdf.pdf" class="file-pdf">Title of PDF</a>.

```md
<a href="/files/test-pdf.pdf" class="file-pdf">Title of PDF</a>
```

**Downloadable files** need to have a `download` attribute added to the anchor. This tells the browser to download the file instead of trying to display it. For example, <a download href="/files/test-zip.zip">Title of ZIP</a>.

```md
<a download href="/files/test-zip.zip">Title of ZIP</a>
```
