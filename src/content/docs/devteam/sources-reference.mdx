...
Here is an example of a link that uses the ```[Link text][@uniquekey]``` syntax - [Apple's AAT technology][@aat]. The `@aat` at the end refers to a source with the key `aat` defined in the `data/sources.yaml` file.
