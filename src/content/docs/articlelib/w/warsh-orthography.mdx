...
It is interesting to note that the _imala e_ (<Character usv="065C" options="usv,char,name"/>) is sometimes referred to as a "warsh dot". The _imala e_ should be large enough that it is not confused with a single _nukat_ below other characters.

## Positioning

Another aspect of Warsh is that the hamza on <Character usv="0623" options="usv,char,name"/> and <Character usv="0625" options="usv,char,name"/> actually touch the alef. To support this orthography, the characters in the range of U+0870..U+0882 were added to Unicode. This range supports a variety of characters such as _fatha_, _kasra_, and _hamza_ touching above, below, or beside an _alef_.
...
In West Africa, where Warsh orthographies are common, the medial form of _feh_ and _qaf_ are written with the stroke of the pen lifted from the page and beginning above the baseline. The first column below is a more standard Arabic font, and the second column is the Warsh-style.

![Medial feh and qaf in Warsh-style (in blue)](images/sch-shadda-kasra-below.png "Medial feh and qaf in Warsh-style (in blue)")
